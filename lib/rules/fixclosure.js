...
module.exports = {
  ...
  create(context) {
    ...
    if (context.options[0] && context.options[0].config) {
      options = resolveConfig({ config: context.options[0].config }).opts;
    } else {
      options = resolveConfig({}).opts;
    }
        ...
        differences.forEach((difference) => {
          switch (difference.operation) {
            ...
            case DELETE:
              ...
              break;
            case REPLACE:
              // We don't replace if the diff is only whether there is a line feed or not
              if (
                difference.deleteText.replace(/\r|\n/g, "") !==
                difference.insertText.replace(/\r|\n/g, "")
              ) {
                reportReplace(
                  context,
                  difference.offset,
